...
| [`src/product.ts`](file:///d:/Projects/Projects%20for%20job/pos/packages/shared-types/src/product.ts) | Defines `ProductDto`, `CreateProductInput`, and `UpdateProductInput` interfaces. Ensures prices are handled as `priceCents` (integer cents) to eliminate floating-point rounding errors. |
...
| [`src/schemas/productSchemas.ts`](file:///d:/Projects/Projects%20for%20job/pos/apps/backend/src/schemas/productSchemas.ts) | Zod validation rules for product creation (`createProductSchema`) and product updates (`updateProductSchema`). Validates positive price cents and stock counts. |
...
| [`src/routes/products.ts`](file:///d:/Projects/Projects%20for%20job/pos/apps/backend/src/routes/products.ts) | Express router mapping `/api/products` endpoints (`GET /`, `GET /:id`, `POST /`, `PUT /:id`, `DELETE /:id`) to `productService` functions. |
...
| [`src/services/productService.ts`](file:///d:/Projects/Projects%20for%20job/pos/apps/backend/src/services/productService.ts) | Service containing business logic for catalog queries, category filtering, stock adjustments, product creation, and updates. |
