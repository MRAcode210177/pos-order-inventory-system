...
export default function StressTestPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider bg-amber-500/15 text-amber-300 border border-amber-500/30">
                Stress Tester
              </span>
            </div>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-white/10 space-y-5 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-300 block">Target Product to Race For</label>
            <select
              value={selectedProductId}
              onChange={(e) => setSelectedProductId(e.target.value)}
              disabled={isRunning}
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-white/10 text-xs text-white focus:outline-none focus:border-brand-500 font-medium"
            >
              {products.map((p) => (
            ...
            {selectedProduct && (
              <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1">
                <span>
                  Current Stock: <strong className="text-brand-400 font-mono">{selectedProduct.stockQuantity}</strong>
                </span>
                <button
                  type="button"
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-300 block">Simultaneous Concurrent Buyers</label>
            <div className="grid grid-cols-3 gap-2">
              {[10, 20, 50].map((count) => (
                <button
                  ...
                  disabled={isRunning}
                  className={cn(
                    'py-2 px-3 rounded-xl text-xs font-bold font-mono transition-all border',
                    concurrentCount === count
                      ? 'bg-amber-500/20 text-amber-300 border-amber-500/40 shadow-sm'
                      : 'bg-slate-900 text-slate-400 border-white/5 hover:border-white/20 hover:text-slate-200'
                  )}
                >
                  {count} Buyers
                </button>
            ...
            <p className="text-[11px] text-slate-500">
              All requests fire at the exact same millisecond via <code className="text-brand-400 font-mono">Promise.all()</code>.
            </p>
          </div>
          ...
          <div className="space-y-2 flex flex-col justify-end">
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-extrabold text-sm flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isRunning ? (
                <>
      ...
      {results.length > 0 && (
        <div className="space-y-5 animate-fadeIn">
          ...
          <div
            className={cn(
              'glass-panel rounded-2xl p-5 border shadow-xl',
              isVerifiedSafe ? 'border-emerald-500/40 bg-emerald-950/10' : 'border-rose-500/40 bg-rose-950/10'
            )}
          >
              ...
              <div className="flex items-center gap-3">
                <div
                  className={cn(
                    ...
                    isVerifiedSafe ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'
                  )}
                >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Execution time: <span className="font-mono text-slate-200">{executionTimeMs}ms</span> across {concurrentCount} parallel connections.
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className="text-xs px-3 py-1 rounded-full font-mono font-bold bg-slate-900 border border-white/10 text-brand-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4">
              <div className="bg-slate-900/70 p-3 rounded-xl border border-white/5">
                <p className="text-[11px] text-slate-400">Total Fired</p>
              ...
              <div className="bg-emerald-950/30 p-3 rounded-xl border border-emerald-500/20">
                <p className="text-[11px] text-emerald-400">201 Success (Reserved)</p>
                <p className="font-mono font-bold text-xl text-emerald-300">{successCount}</p>
              </div>
              <div className="bg-rose-950/30 p-3 rounded-xl border border-rose-500/20">
                <p className="text-[11px] text-rose-400">409 Out of Stock (Blocked)</p>
                <p className="font-mono font-bold text-xl text-rose-300">{failedCount}</p>
              </div>
              <div className="bg-slate-900/70 p-3 rounded-xl border border-white/5">
                <p className="text-[11px] text-slate-400">Final DB Stock</p>
                <p className="font-mono font-bold text-xl text-brand-400">{finalStockAfterRun}</p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel rounded-2xl border border-white/10 overflow-hidden">
            <div className="p-4 bg-slate-900/80 border-b border-white/10 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400 font-mono">{results.length} requests processed</span>
            </div>

            <div className="max-h-96 overflow-y-auto divide-y divide-white/5 p-2 space-y-1">
              {results.map((res) => (
                <div
                  key={res.buyerIndex}
                  className={cn(
