...
export function CheckoutModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-lg glass-panel rounded-2xl border border-border shadow-2xl overflow-hidden animate-scaleUp">
        {/* Modal Header */}
        <div className="p-5 border-b border-border flex items-center justify-between bg-card/80">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-mono">Order ID: {order.id.substring(0, 12)}...</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {!completedPayment && (
            <div className="flex items-center justify-between p-3 rounded-xl bg-card border border-primary/20">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-primary" />
                <span className="text-xs text-foreground font-medium">Inventory Reserved</span>
              </div>
          ...
          {completedPayment ? (
            <div className="space-y-5 text-center py-2">
              <div className="w-16 h-16 mx-auto rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-emerald-500 shadow-glow">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Transaction completed with verified stock</p>
              </div>

              {/* Receipt Summary Card */}
              <div className="p-4 rounded-xl bg-card border border-border text-left space-y-2.5 text-xs shadow-sm">
                ...
                <div className="flex justify-between text-muted-foreground">
                  <span>Status</span>
                  <span className="text-emerald-500 font-semibold uppercase">PAID & DEDUCTED</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                ...
                <div className="flex justify-between text-muted-foreground">
                  <span>Timestamp</span>
                  <span className="text-foreground">{formatDateTime(completedPayment.createdAt)}</span>
                </div>
                <div className="pt-2 border-t border-border space-y-1">
                  <span className="text-[11px] text-muted-foreground font-medium">Line Items:</span>
                  {order.items.map((i, idx) => (
                    <div key={idx} className="flex justify-between text-foreground">
                      <span>
                        {i.quantity}x {i.productName || i.productId.substring(0, 8)}
                      </span>
                      <span className="font-mono">{formatCurrency(i.unitPriceCents * i.quantity)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <button
                onClick={onClose}
                className="w-full py-3 px-4 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-sm shadow-glow transition-all"
              >
            ...
            <form onSubmit={handlePay} className="space-y-4">
              ...
              <div className="p-3.5 rounded-xl bg-muted/60 border border-border space-y-2">
                <div className="flex justify-between text-xs font-semibold text-foreground">
                  ...
                  <span className="text-primary font-bold">{formatCurrency(order.totalCents)} Total</span>
                </div>
              ...
              {/* Error Alert */}
              {errorMessage && (
                <div className="p-3 rounded-xl bg-destructive/15 border border-destructive/30 text-destructive text-xs flex items-start gap-2.5">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <label className="font-medium text-foreground">Idempotency Key</label>
                  <button
                    type="button"
                ...
                <div className="relative flex items-center">
                  <input
                    ...
                    required
                    className="w-full px-3.5 py-2.5 rounded-xl bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary text-xs font-mono text-foreground outline-none pr-10"
                  />
                  <button
                    type="button"
              ...
              <div className="pt-2 flex items-center gap-3">
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 py-2.5 px-4 rounded-xl bg-card hover:bg-accent text-muted-foreground hover:text-foreground text-xs font-semibold border border-border transition-all"
                >
